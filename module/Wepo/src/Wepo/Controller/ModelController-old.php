...
use ModelFramework\DataMapping;
...
use MonZend\Db\Adapter\Platform\Mongo;
use Wepo\Lib\WepoController;
use Wepo\Model\Status;
use Zend\View\Model\ViewModel;
use Zend\Console\Request as ConsoleRequest;

class ModelControllerBak extends WepoController
{
...
//                prn($gw->find()->toArray(),$dbsName);
    ...
    public function dbcmprAction()
    {
        ...
        $results[ 'title' ] =
            'Looking over "' . $results[ 'from' ] . '" database ';
...
//        prn($config);
        ...
        $systemConfig[ 'db' ] = $this->getGatewayServiceVerify()
...
//
            $setting     = $gw->find( [
                ...
                '_models.from' => $results[ 'from' ]
                              ...
                              ->getArrayCopy();
            $allowedKeys =
                array_filter( array_keys( $setting ), function ( $key ) {
...
//            prn('is id',$modelIdFields);
            ...
            foreach ($results[ 'modelCMPR' ] as $from => $to) {
                ...
                if (strlen( $from ) && strlen( $to )) {
                    ...
                    $field =
                        implode( '_', explode( ' ', strtolower( $from ) ) );
                    if (isset( $results[ 'idlinks' ][ $idfield ] )) {
                        $dbase            = $results[ 'idlinks' ][ $idfield ];
                        $setting[ $from ] = [
                            'to'   => $field,
...
//        $results->setTemplate('wepo\model\sort.twig');
    ...
    public function triggerLogic( $gwConvert, $logicType, $modelName )
    {
        ...
        $count  = 0;
        $models =
            $this->getServiceLocator()->get( 'ModelFramework\GatewayService' )
...
//            prn( 'end logic update' );
    ...
    private function checkPermission()
    {
        ...
        if (isset( $db )) {
            ...
            if ($request instanceof ConsoleRequest) {
                ...
                if ($check->count()) {
                    ...
                    if (in_array( (string) $mainUser->status_id,
                        ...
                        $this->getServiceLocator()
                             ->get( 'ModelFramework\LogicService' )
            ...
            $main_user =
                $this->table( 'MainUser' )->get( $this->user()->main_id );
            $dbsName   = $this->table( 'MainDb' )
                              ->find( [ 'company_id' => $main_user->company_id ] )
                              ->toArray()[ 0 ][ 'dbname' ];
        ...
        foreach ($rules as $rule) {
            if (( count( $rule ) > 3 )
                ...
                $gwSrc = new MongoGateway( $rule[ '_models' ][ 'from' ],
                    $dbAdapterCSV );
                $gwTar = $this->table( $rule[ '_models' ][ 'to' ] );

                $result = $gwSrc->fetchAll();
//                prn($rule[ '_models' ][ 'from' ]);
//                prn(count($result));
//                exit;
                ...
                foreach ($result as $srcModel) {
                    ...
                    $trgModel =
                        new \ArrayObject( $this->getModelServiceVerify()
                                               ->get( $rule[ '_models' ][ 'to' ] )
                                               ->toArray(),
...
//                                look in lookup field settings and create additional fields
...
//                    $gwTar->delete( [ '_id_export' => $trgModel[ '_id_export' ] ] );
...
//        prn($supportedModels);
        foreach ($exportSettings->toArray() as $exportSetting) {
            if (in_array( $exportSetting[ '_models' ][ 'to' ],
                ...
                $curWepoModelGw
                    = new MongoGateway( $this->model( $curWepoModelName )
...
//                                //activity check block finished
                                ...
                                if ($exportSetting[ '_models' ][ 'from' ]
                                    ...
                                    if (isset( $extWepoModel )
                                        ...
                                        $extWepoModelGw
                                            =
                                            new MongoGateway( $this->model( $extWepoModel )
                                                                   ->getTableName(),
                                                $curDbAdapter );
                                        $sourceWepoModel
                                            = $extWepoModelGw->find( [
...
//                                prn( 'static_lookup' );
                                ...
                                $curWepoField        = $toField[ 'to' ];
                                $curModelIdFieldData =
                                    $curWepoModel->{$curWepoField . '_export'};
...
//        prn($exportSettings);
        ...
        foreach ($exportSettings as $setting) {
            if (in_array( $setting[ '_models' ][ 'to' ], $supportedModels )) {
                ...
                $curWepoModelGw
                    =
                    new MongoGateway( $this->model( $setting[ '_models' ][ 'to' ] )
                                           ->getTableName(), $curDbAdapter );

                $data = $curWepoModelGw->fetchAll();
                $data = $data->toArray();

                $gw = $this->table( $setting[ '_models' ][ 'to' ] );
//                $data = $gw->find( [ ] );
                prn( $setting[ '_models' ][ 'to' ] );
                $count = 0;
                foreach ($data as $model) {
                    ...
                    $id       = $model[ '_id' ];
                    $model    = $this->model( $setting[ '_models' ][ 'to' ] );
...
//            exit;
    ...
    public function productlink( $modelName )
    {
        ...
        if (in_array( $modelName, $allowedModels )) {
            ...
            $productGW   = $this->table( 'Product' );
            $pricebookGW = $this->table( 'PricebookDetail' );
...
//                prn($pricebookdetail->_id);
    ...
    public function activity( $modelName )
    {
        ...
        foreach ($activities as $activity) {
            ...
            if ($flag) {
                ...
                $activity     =
                    array_merge( $this->getModelServiceVerify()
                        ...
                        $activity );
                $activity     =
                    new \ArrayObject( $activity, \ArrayObject::ARRAY_AS_PROPS );
            }
            ...
            $this->table( $resModelName )->save( $temp );
        }
    }
